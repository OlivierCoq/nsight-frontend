    ...
    async login(payload) {
      ...
      if (res.statusCode === 400) {
        ...
        const custom_data = await $fetch(
          ...
          $fetch(`${runtimeConfig.public.NUXT_STRAPI_URL}/api/profiles?${qs.stringify({
            populate: ["users_permissions_user", "users_permissions_user.nsight_id"],
            filters: {
            ...
            if(!res?.data?.length) {
              ...
              $fetch(`${runtimeConfig.public.NUXT_STRAPI_URL}/api/profiles`, {
                ...
                body: JSON.stringify({
                  users_permissions_user: this.user.id,
                  nsight_id: this.user.nsight_id.id,
                }),
              }).then(async (res) => {
