<template>
  <div
    v-if="state?.product"
    class="h-[100vh] w-full bg-zinc-200 dark:bg-zinc-900 flex flex-col px-3 pb-4 pt-16"
  >
    <div class="container mx-auto p-3 flex flex-col md:flex-row">
      ...
      <div v-else class="w-full md:w-1/2 p-3 flex flex-col">
        <img
          class="w-full shadow-xl rounded-md"
          :src="state?.selected?.images[0]?.url"
        />
        <div class="py-2">
          <PrimeCarousel
            v-if="state?.product?.images?.length"
            :value="state?.product?.images"
            :numVisible="3"
            circular
            :numScroll="3"
          ...
          >
            <template #item="slotProps">
              <div
                class="w-[190px] h-[135px] mx-2 bg-cover bg-center rounded-md shadow-lg hover:cursor-pointer"
                @click="slotProps.data.modal = true"
                :style="{
                  backgroundImage: `url(${slotProps.data.imageData.url})`,
                }"
              >
                <PrimeDialog
                  v-model:visible="slotProps.data.modal"
                  :style="{ width: '50rem' }"
                >
                  <div
                    class="w-full bg-center bg-cover h-[50rem]"
                    :style="{
                      backgroundImage: `url(${slotProps.data.imageData.url})`,
                    }"
                  ></div>
      ...
      <div
        class="w-full md:w-1/2 p-6 bg-zinc-300 dark:bg-zinc-800 rounded-md shadow-xl mx-3"
      >
        <div class="w-3/4 flex flex-row justify-between">
          <h1
            class="text-3xl text-neutral-900 dark:text-neutral-200 font-thin pb-3"
          >
            {{ state.product?.item?.itemData?.name }}
          </h1>
          <h2
            class="text-xl text-neutral-900 dark:text-neutral-200 font-thin"
            v-html="
              format_currency(
        ...
        <p class="text-lg text-neutral-900 dark:text-neutral-200 opacity-85">
          {{ state.product?.item?.itemData?.description }}
        </p>
        ...
        <div class="w-full md:w-1/4 mt-5">
          <label
            for="quantity"
            class="text-md text-neutral-900 dark:text-neutral-200 font-thin"
            >Quantity</label
          >
          <input
            type="number"
        ...
        <div class="w-full flex flex-row justify-start pb-5 items-center">
          <PrimeToast />
          <button
            v-if="state.product?.item"
            class="btn-add_to_cart nsight-btn-primary my-1 py-1 w-full md:w-1/2 px-5 text-white shadow-xl rounded-md"
            @click="add_to_cart"
          >
            Add to cart
          </button>
        ...
        <div class="w-full mt-5 mb-10 flex flex-row items-start">
          <h2 class="text-md text-neutral-900 dark:text-neutral-200 font-thin">
            Categories:
          </h2>
          <div class="flex flex-row items-center">
            <p
              v-for="category in state.product?.categories"
              :key="category.id"
        ...
        <div
          v-if="state.product?.item?.itemData?.variations?.length"
          class="w-full mb-8 flex flex-col items-start justify-start"
        >
          <h2 class="text-md text-neutral-900 dark:text-neutral-200 font-thin">
            Styles :
            <strong>{{ state.selected?.itemVariationData?.name }}</strong>
          </h2>
          <div class="w-full grid grid-cols-6">
            <div
              v-for="(variation, a) in state.product?.item?.itemData
                ?.variations"
              :key="a"
              class="w-[75px] h-[75px] my-4 bg-cover bg-center rounded-md shadow-lg hover:cursor-pointer"
              :class="[
                state.selected?.id === variation.id
